// Copyright (C) 2007 Hendrik Baecker <andurin@process-zero.de>
// Inspired by Ethan Galstad <nagios@nagios.org>
//
...
//
// $Id: config.c 440 2008-04-24 09:08:20Z hendrikb $
...
void process_configfile(char *config_file)
{
    ...
    /* ok - parsing the config file line by line */
        ...
        while (feof(fh_config_file) == 0) {
            ...
            if (current_config_line[0] == '#'
                || current_config_line[0] == '\n')
                continue;

            strip(current_config_line);
            ...
            if (!strcmp(variable, "log_file")) {
                ...
                strip(macro_x[CONFIG_OPT_LOGFILE]);
            }
            ...
            /* log_file_size directive */
                ...
                if (strlen(value) == 1) {
                    if (isdigit((int) value[strlen(value) - 1]) == FALSE) {
                        strcpy(error_msg,
                               "log_file_size should be an integer");
                        error = TRUE;
                        break;
                    }
                }
                ...
                strip(macro_x[CONFIG_OPT_LOGFILESIZE]);
            }
            ...
            /* log_level directive */
                ...
                if (strlen(value) == 1) {
                    if (isdigit((int) value[strlen(value) - 1]) == FALSE) {
                        strcpy(error_msg,
                               "log_level should be an integer");
                        error = TRUE;
                        break;
                    }
                }
                ...
                strip(macro_x[CONFIG_OPT_LOGLEVEL]);
            }
            ...
            /* log_type directive */
                ...
                strip(macro_x[CONFIG_OPT_LOGTYPE]);
                ...
                strip(macro_x[CONFIG_OPT_GROUP]);
            }
                ...
                strip(macro_x[CONFIG_OPT_RUNCMD]);
            }
                ...
                strip(macro_x[CONFIG_OPT_RUNCMD_ARG]);
            }
                ...
                strip(macro_x[CONFIG_OPT_MAXTHREADS]);
            }

            else if (!strcmp(variable, "use_load_threshold")) {

                /* save the macro */
                if (macro_x[CONFIG_OPT_USELOAD] != NULL)
                    free(macro_x[CONFIG_OPT_USELOAD]);

                macro_x[CONFIG_OPT_USELOAD] = (char *) strdup(value);

                if (macro_x[CONFIG_OPT_USELOAD] == NULL) {
                    strcpy(error_msg,
                           "Could not allocate memory for macro USELOAD");
                    error = TRUE;
                    break;
                }
                strip(macro_x[CONFIG_OPT_USELOAD]);
            }
                ...
                strip(macro_x[CONFIG_OPT_PIDFILE]);
                ...
                if (strlen(value) == 1) {
                    if (isdigit((int) value[strlen(value) - 1]) == FALSE) {
                        strcpy(error_msg,
                               "sleep_time should be an integer");
                        error = TRUE;
                        break;
                    }
                }
                ...
                strip(macro_x[CONFIG_OPT_SLEEPTIME]);
            }
...
int prepare_vars()
{
    ...
    if (macro_x[CONFIG_OPT_USELOAD] != NULL) {
        use_load_threshold = atoi(macro_x[CONFIG_OPT_USELOAD]);
    } else
