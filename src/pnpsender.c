// Copyright (C) 2006-2008 Hendrik Baecker <andurin@process-zero.de>
//
...
//
// $Id: pnpsender.c 441 2008-04-24 17:52:34Z pitchfork $
...
int process_arguments(int argc, char **argv)
{
...
#ifdef HAVE_GETOPT_H
        c = getopt_long(argc, argv, "+hH:p:d:Vt:", long_options,
                        &option_index);
#else
    ...
    if (error == TRUE || display_help == TRUE) {
        ...
        printf("\n");
        printf
            ("  -d | --datatype Free string what this Data stands for\n");
        printf
...
void print_list(int socket, struct Nagios_Env *base)
{
    ...
    int message_length = sizeof(struct Nagios_Env);
    ptr = base;
    strcat(dtype,
    ...
    if (strncmp(var1, (char *) "NAGIOS_", 7) == 0) {
        if (var2 != NULL) {
            if (ptr == NULL) {
                ...
                ptr =
                    (struct Nagios_Env *)
                    malloc(sizeof(struct Nagios_Env));
                if (ptr == NULL)
                    return ERROR;
                *base = ptr;
            } else {
...
int open_sock()
{
    ...
    /* connect to server */
    if (connect(tosocket, (struct sockaddr *) &toaddr, sizeof(toaddr)) ==
        -1) {
        fprintf(stderr,
