// Copyright (C) 2007-2008 Hendrik Baecker <andurin@process-zero.de>
//
...
//
// $Id: npcd.c 440 2008-04-24 09:08:20Z hendrikb $
//
...
int main(int argc, char **argv)
{
    ...
    if (loglevel == -1) {
        ...
        printf("CONFIG_OPT_USELOAD = %s\n", macro_x[CONFIG_OPT_USELOAD]);
...
#else */
        if ((filecounter =
             scandir(directory, &namelist, 0, alphasort)) < 0) {
/* #endif */
            snprintf(buffer, sizeof(buffer) - 1,
        ...
        for (i = 0, namelist; i < filecounter; i++) {
            ...
            if (S_ISREG(attribute.st_mode)) {
                ...
                if (thread_counter < max_threads
                    ...
                    if ((pthread_ret =
                         pthread_create(&th[thread_counter], NULL,
                                        processfile,
                                        namelist[i]->d_name)) != 0) {
                        LOG(0, "Could not create thread... exiting\n");
                        exit(EXIT_FAILURE);
                    }

                    snprintf(buffer, sizeof(buffer) - 1,
                             "A thread was startet on thread_counter = %d\n",
                             thread_counter);
                    LOG(2, buffer);
                    ...
                    for (thread_counter = thread_counter;
                         thread_counter > 0; thread_counter--)
        ...
        if (thread_counter > 0) {
            ...
            for (thread_counter = thread_counter; thread_counter > 0;
                 thread_counter--)
                pthread_join(th[thread_counter - 1], NULL);
        }
...
int process_arguments(int argc, char **argv)
{
    ...
    if (error == TRUE || display_help == TRUE) {
        ...
        printf
            ("new releases, online documentation, FAQs, Mailinglists.\n");
        printf("\n");
...
void *processfile(void *filename)
{
    ...
    snprintf(command_line, 255, "%s %s %s/%s", command, command_args,
             directory, file);
