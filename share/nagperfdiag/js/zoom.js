...
function zoomGraphObjRefresh() {
	...
	if (titleFontSize == 0) {
		...
		var cZoomBoxTopOffsetWOText = titleFontSize + (titleFontSize * 1.6) + 10 - 2;
...
function onMouseUpEvent(e) {
	...
	if ((gMouseObj.rightButtonPressed()) && (insideZoomBox())) {
		...
		open(urlBase + "&host=" + host + "&srv=" + srv + "&view=" + view + "&source=" + source + "&start=" + newGraphStart + "&end=" + newGraphEnd + "&graph_height=" + graphHeight + "&graph_width=" + graphWidth + "&title_font_size=" + titleFontSize, "_self");
	}
